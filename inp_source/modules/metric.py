...
class LabelMetric(Metric):
    def __init__(self, n_labels,eps=1e-8):
        ...
        # [predicts, golds]
        self.matrix = torch.zeros((n_labels, n_labels), device=device)
    ...
    def __call__(self, predicts, golds):
        ...
        """
        self.total += len(predicts)
        self.correct += predicts.eq(golds).sum()

        predicts = one_hot(predicts, num_classes=self.n_labels).unsqueeze(-1)
        golds = one_hot(golds, num_classes=self.n_labels).unsqueeze(-2)
        self.matrix += (predicts * golds).sum(0)
