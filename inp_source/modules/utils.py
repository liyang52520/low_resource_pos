...
def sents_to_vec(vec_dict, sentences):
    ...
    for sent in sentences:
        ...
        sequence = [word for word in sent["word"]]
...
def read_conll(fname, max_len=1e3, rm_null=True, prc_num=True, ud=False):
    sentences = []
    sent = ConllSent()
        ...
        for line in fin:
            if line != '\n':
                ...
                sent["head"].append((int(line[0]),
                                     cast_to_int(line[3])))
                if rm_null and line[tag_idx] == '-NONE-':
                    null_sent.append(loc)
                else:
                    sent["tag"].append(line[tag_idx])
...
def data_iter(data, batch_size, is_test=False, shuffle=True):
    ...
    for i in range(batch_num):
        ...
        if is_test:
            ...
            words = [data_tuple[1] for data_tuple in batch_data]
            tags = [data_tuple[2] for data_tuple in batch_data]

            yield test_data, words, tags
